  ...
  const plans = [
    {
      ...
      color: "border-border-subtle dark:border-slate-800/80 bg-white bg-bg-primary",
      badgeColor: "bg-surface-glass bg-surface-solid text-slate-700",
      icon: "⚡",
      features: [
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 fade-in">
      ...
      <div className="mb-12 text-center">
        <span className="rounded-full bg-blue-50 px-3.5 py-1 text-xs font-bold uppercase tracking-widest text-accent">
          Simple, transparent pricing
        </span>
        <h1 className="mt-3 font-display text-3xl font-black text-white text-white md:text-4.5xl tracking-tight">
          Choose the plan that's right for you
        </h1>
        <p className="mt-2 text-sm text-gray-500 text-text-secondary max-w-md mx-auto">
        ...
        <div className="mt-6 flex flex-col sm:flex-row justify-center items-center gap-4">
          <div className="inline-flex items-center gap-1.5 rounded-xl bg-surface-glass bg-surface-solid p-1">
            <button
              onClick={() => setBillingCycle("monthly")}
              className={`rounded-lg px-4 py-1.5 text-xs font-bold transition-all ${
                billingCycle === "monthly" ? "bg-white bg-bg-primary text-white text-white shadow-xs" : "text-gray-500 text-text-secondary hover:text-gray-900 text-white"
              }`}
            >
              Monthly billing
            </button>
            <button
              ...
              className={`rounded-lg px-4 py-1.5 text-xs font-bold transition-all ${
                billingCycle === "annual" ? "bg-white bg-bg-primary text-white text-white shadow-xs" : "text-gray-500 text-text-secondary hover:text-gray-950"
              }`}
            >
              Annual billing
            </button>
            ...
            >
              ...
              <div>
                ...
                <div className="mt-3 flex items-baseline">
                  <span className="font-display text-3.5xl font-black tracking-tight text-white text-white">
                    ₹{priceToShow}
                  </span>
                ...
                <ul className="mt-6 space-y-3 border-t border-gray-100 dark:border-slate-800 pt-5 text-left">
                  {p.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-xs text-gray-600 text-text-secondary">
              ...
              <div className="mt-8 pt-2">
                {isActive ? (
                  <div className="rounded-xl bg-surface-solid bg-bg-primary font-bold p-3 text-center text-xs text-gray-400 border border-border-subtle uppercase tracking-widest font-mono select-none">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-center gap-1.5 rounded-xl py-3 text-xs font-bold transition cursor-pointer select-none border ${
                      p.isPopular 
                        ...
                        : "bg-surface-solid bg-bg-primary hover:bg-surface-glass bg-surface-solid text-white text-white border-border-subtle dark:border-slate-800/80"
                    }`}
                  >
      ...
      <div className="mt-20 border-t border-gray-250 pt-16">
        <h2 className="text-center font-display text-2xl font-black text-white text-white tracking-tight md:text-3xl mb-8">
          Frequently asked questions
        </h2>
        <div className="mx-auto max-w-3xl grid gap-6 md:grid-cols-2">
          {[
            ...
            <div key={i} className="rounded-2xl border border-border-subtle dark:border-slate-800/60 p-6 bg-white bg-bg-primary shadow-3xs">
              <h4 className="font-extrabold text-sm text-white text-white">{faq.q}</h4>
      ...
      {loadingStep >= 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4 backdrop-blur-xs fade-in">
          <div className="max-w-md w-full bg-white bg-bg-primary rounded-3xl p-6 shadow-2xl relative border border-border-subtle">
            
            {!showReceipt ? (
              <div className="text-center py-10 space-y-4">
                <Loader2 className="mx-auto h-12 w-12 text-accent animate-spin" />
                <h3 className="font-display text-xl font-bold text-white text-white">Securing payment portal</h3>
                <p className="text-xs text-gray-500 text-text-secondary italic max-w-xs mx-auto">
                ...
                <div className="w-full bg-surface-glass bg-surface-solid h-1.5 rounded-full overflow-hidden">
                  <div 
              ...
              <div className="space-y-4 text-center fade-in">
                ...
                <h3 className="font-display text-2xl font-black text-white text-white">Payment Approved!</h3>
                <p className="text-xs text-gray-500 text-text-secondary font-medium">Your license is securely upgraded. Below is your compiled invoice:</p>

                {/* Stylized academic purchases invoice */}
                <div className="rounded-2xl border border-dashed border-gray-200 dark:border-slate-800 bg-surface-solid bg-bg-primary/50 p-5 text-left text-xs font-mono space-y-2">
                  ...
                  <div className="flex justify-between text-gray-550">
                    ...
                    <span className="text-right text-accent font-bold">{selectedPlan} Scholar License</span>
                  </div>
                  <div className="flex justify-between text-gray-500 text-text-secondary">
                    <span>Duration</span>
                  ...
                  <div className="flex justify-between font-bold text-sm text-white text-white border-t border-dashed border-gray-150 pt-2 mt-2 leading-none">
                ...
                <div className="flex gap-2.5">
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 rounded-xl border border-gray-200 dark:border-slate-800 hover:bg-surface-solid bg-bg-primary px-4 py-3 text-xs font-bold text-gray-600 text-text-secondary transition"
                  >
                  ...
                  <button
                    ...
                    className="grow flex items-center justify-center gap-1.5 rounded-xl bg-brand-navy hover:bg-accent py-3 text-xs font-black text-white shadow-md transition cursor-pointer leading-none"
                  >
