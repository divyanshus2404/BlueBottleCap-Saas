    ...
    <div className="fixed bottom-5 right-5 z-[9999] flex flex-col gap-2.5 pointer-events-none" style={{ maxWidth: 340 }}>
      {toasts.map((toast) => {
        ...
        return (
          <div
            key={toast.id}
            className={`pointer-events-auto flex items-start gap-3 rounded-xl bg-white bg-bg-primary shadow-lg shadow-slate-200/80 px-4 py-3 text-sm text-slate-700 font-medium animate-in slide-in-from-right-4 fade-in duration-200 ${borders[toast.type]}`}
          >
            {icons[toast.type]}
            <span className="leading-snug flex-1">{toast.message}</span>
            <button onClick={() => onDismiss(toast.id)} className="shrink-0 text-gray-300 hover:text-gray-500 text-text-secondary transition cursor-pointer ml-1">
